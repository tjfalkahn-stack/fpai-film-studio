...
`vibes-manual` is a first-class catalog option in **Generate Take**, but it is intentionally not a server-side renderer. Film Studio selects one Primary Identity reference, prepares a continuity-aware prompt, and offers three owner actions: open `https://vibes.ai/` while copying the prompt, copy only the prompt, or download a JSON handoff containing the shot request and selected-reference manifest.

After generation, download the MP4 from Vibes and use the existing **Upload completed take** control. The take then follows the normal continuity score, shortlist, approval, salvage, and ledger workflow.

The adapter reports a `$0.00` Film Studio charge because no FPAI renderer is called. This is not a promise that Meta will keep Vibes free or unlimited. No Vibes login, cookie, credential, or production secret is stored. Direct POST submission with `provider=vibes-manual` fails closed with `MANUAL_PROVIDER`; Vibes cannot enter the paid queue or bypass live-rendering controls.

The generation contract always preserves the full ordered selection (asset IDs, reasons, lock version) on the render. **Veo Fast still accepts at most three PNG/JPEG images.** Seedance accepts up to nine Character Bible / scene references and can consume a reference video or end frame. Mock records the full set (primary + up to five supporting by default; `CHARACTER_REFERENCE_SUPPORTING_LIMIT`) and does not pretend extra images were sent to a live model. If an executable provider has `maxReferences: 1`, only the Primary Identity image is transmitted. Vibes is manual, so its Primary Identity selection stays manifest-only until the owner uploads it in the Vibes tab.
