...
const KEY="fpai-film-studio-v1.1D", DB="fpai-film-studio-media", STORE="media";
const REFS=[["masterFace","MASTER FACE"],["fullBody","FULL BODY"],["profile","PROFILE"],["wardrobe","WARDROBE"],["rain","RAIN / WET"]];
const EXPRESSIONS=["Neutral","Suspicious","Controlled Anger","Hurt","Paternal","Exhausted"];

const seed={
 project:{title:"ENEMIES CLOSER",subtitle:"IYKYK · EP01 · Houston crime thriller",runtime:1620,budget:1200,openingBudget:165,drift:"STRICT"},
 characters:[
...
function migrate(){
 let old=null;
 for(const k of[KEY,"fpai-film-studio-v1.1C","fpai-film-studio-v1.1B","fpai-film-studio-v1.1A"]){
  try{old=JSON.parse(localStorage.getItem(k));if(old)break}catch{}
 }
 if(!old)return seed;
 return {...seed,...old,
  project:{...seed.project,...old.project},
  characters:(old.characters||seed.characters).map(c=>({...c,refs:c.refs||{},expressions:c.expressions||{}})),
...
function App(){
 const[data,setData]=useData(),[tab,setTab]=useState("Overview"),[char,setChar]=useState(null),[shot,setShot]=useState(null),[pkg,setPkg]=useState(null),[view,setView]=useState("Storyboard"),[dragTarget,setDragTarget]=useState(null);
 ...
 const spent=useMemo(()=>data.ledger.reduce((n,x)=>n+Number(x.amount||0),0),[data.ledger]);
 ...
 function validate(s){
  ...
  s.characters.forEach(id=>{const x=c(id);if(!x?.locked)blockers.push(`${x?.name||id} NOT LOCKED`);if(x?.locked)REFS.forEach(([r,l])=>{if(!x.refs?.[r])blockers.push(`${x.name} missing ${l}`)})});
 ...
 async function addRef(ch,slot,file){if(!file)return;const key=`char:${ch.id}:${slot}:${Date.now()}`;await putMedia(key,file);const refs={...ch.refs,[slot]:{key,name:file.name}};updateChar(ch.id,{refs,locked:false});setChar({...ch,refs,locked:false})}
 ...
 {tab==="Characters"&&<div className="panel"><span className="eyebrow">VISUAL BIBLE</span><h2>Canonical Character Locks</h2><div className="charactergrid">{data.characters.map(x=><div className="charcard" key={x.id}><div className="portrait">{x.refs?.masterFace?.key?<Media mediaKey={x.refs.masterFace.key}/>:x.name[0]}</div><h3>{x.name}</h3><small>{x.role}</small><p>{Object.keys(x.refs||{}).length}/5 required reference assets</p><span className={x.locked?"ok":"draft"}>{x.locked?"LOCKED":"NOT LOCKED"}</span><button className="ghost full" onClick={()=>setChar(x)}>Open character bible</button></div>)}</div></div>}
 ...
 {tab==="Budget"&&<section className="two"><div className="panel"><span className="eyebrow">PRODUCTION LEDGER</span><h2>$1,200 Hard Ceiling</h2><div className="bigmoney">${spent.toFixed(2)}<small>spent</small></div><div className="budgetrows"><div><span>Opening POC</span><b>$165</b></div><div><span>Opening remaining</span><b>${Math.max(0,165-spent).toFixed(2)}</b></div><div><span>Full film remaining</span><b>${Math.max(0,1200-spent).toFixed(2)}</b></div><div><span>Approved footage</span><b>{fmt(approved)}</b></div></div></div><div className="panel"><span className="eyebrow">TRANSACTIONS</span><h2>Actual Spend</h2>{data.ledger.length?data.ledger.map(x=><div className="ledgerrow"><span>{x.date} · {x.label}</span><b>${x.amount.toFixed(2)}</b></div>):<p className="sub">No spend logged.</p>}</div></section>}
 </main>

 {char&&<div className="overlay"><div className="drawer wide"><button className="close" onClick={()=>setChar(null)}><X/></button><span className="eyebrow">CHARACTER BIBLE</span><h2>{char.name}</h2><h3>Required Visual References</h3><p className="dropHint">Drag an image onto a slot, or click the slot to browse.</p><div className="refgrid">{REFS.map(([slot,label])=>{const ref=char.refs?.[slot];const key=`ref:${slot}`;return <label className={`refslot dropzone ${dragTarget===key?"dragging":""}`} onDragEnter={e=>{e.preventDefault();setDragTarget(key)}} onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect="copy"}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setDragTarget(null)}} onDrop={e=>{e.preventDefault();setDragTarget(null);const f=e.dataTransfer.files?.[0];if(f?.type?.startsWith("image/"))addRef(char,slot,f)}}><div className="refpreview">{ref?<Media mediaKey={ref.key}/>:<><Upload/><span>DROP IMAGE</span></>}</div><b>{label}</b><small>{ref?ref.name:"Required before lock"}</small><div className="slotAction">{ref?"DROP TO REPLACE · CLICK TO BROWSE":"DROP HERE · CLICK TO BROWSE"}</div><input type="file" accept="image/*" onChange={e=>addRef(char,slot,e.target.files?.[0])}/></label>})}</div><h3>Expression Bank</h3><p className="dropHint">Expressions support drag-and-drop too.</p><div className="expressiongrid">{EXPRESSIONS.map(n=>{const key=`expr:${n}`;return <label className={`expression dropzone ${dragTarget===key?"dragging":""}`} onDragEnter={e=>{e.preventDefault();setDragTarget(key)}} onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect="copy"}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setDragTarget(null)}} onDrop={e=>{e.preventDefault();setDragTarget(null);const f=e.dataTransfer.files?.[0];if(f?.type?.startsWith("image/"))addExpr(char,n,f)}}><div>{char.expressions?.[n]?<Media mediaKey={char.expressions[n].key}/>:<><Upload/><span>DROP</span></>}</div><b>{n}</b><input type="file" accept="image/*" onChange={e=>addExpr(char,n,e.target.files?.[0])}/></label>})}</div><label>Role<input value={char.role} readOnly/></label><label>Voice<input value={char.voice} onChange={e=>{const v=e.target.value;setChar({...char,voice:v});updateChar(char.id,{voice:v})}}/></label><label>Wardrobe<input value={char.wardrobe} onChange={e=>{const v=e.target.value;setChar({...char,wardrobe:v});updateChar(char.id,{wardrobe:v})}}/></label><label>Continuity notes<textarea value={char.notes} onChange={e=>{const v=e.target.value;setChar({...char,notes:v});updateChar(char.id,{notes:v})}}/></label>{Object.keys(char.refs||{}).length<5&&<div className="validation"><AlertTriangle/>Missing {5-Object.keys(char.refs||{}).length} required reference(s).</div>}<button disabled={Object.keys(char.refs||{}).length<5} className="primary full" onClick={()=>{const locked=!char.locked;updateChar(char.id,{locked});setChar({...char,locked})}}><LockKeyhole/>{char.locked?"UNLOCK CHARACTER":"LOCK CHARACTER"}</button></div></div>}
